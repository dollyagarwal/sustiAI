...
try:
    ...
    # pragma: no cover - graceful fallback when MCP SDK is not available
    FastMCP = None

DB_PATH = "event_emissions.db"

# Initialize FastMCP (if available). If the MCP package is not installed we
# provide a tiny shim so the module can still be imported in CI / test
# environments. The core logic of the reporting tools remains unchanged.
if FastMCP is not None:
    ...
    class _DummyMCP:
        ...
        def run(self, *a, **kw):
            raise RuntimeError("MCP tools not available in this environment")
